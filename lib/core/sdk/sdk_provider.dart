...
class GreenOrangeSdkService implements SdkService {
  ...
  Future<List<Map<String, dynamic>>> scanDevices({String? uuid, String? mac}) async {
  ...
  @override
  Stream<VitalSample<int>> heartRateStream(String deviceId) => const Stream.empty();
  ...
  @override
  Stream<VitalSample<double>> temperatureStream(String deviceId) => const Stream.empty();
  ...
  @override
  Stream<VitalSample<int>> batteryStream(String deviceId) => const Stream.empty();
  ...
  @override
  Stream<VitalSample<int>> respirationStream(String deviceId) => const Stream.empty();
  ...
  @override
  Stream<VitalSample<BloodPressure>> bloodPressureStream(String deviceId) => const Stream.empty();
}
...
class MockSdkService implements SdkService {
  ...
  StreamController<VitalSample<T>> _ctrl<T>(String id, String key, StreamController<VitalSample<T>> Function() create) {
    _controllers[id] ??= {};
  ...
  void _tick<T>(String id, StreamController<VitalSample<T>> c, T Function() gen, [int ms = 1000]) async {
  ...
  Future<void> startHeartRateNotifications(String id) async {
    final c = _ctrl<int>(id, 'hr', () => StreamController<VitalSample<int>>.broadcast());
    if (!c.hasListener) _tick<int>(id, c, () => 60 + _rnd.nextInt(40));
  }
  @override Future<void> stopHeartRateNotifications(String id) async => (_controllers[id]?['hr'] as StreamController?)?.close();
  @override Stream<VitalSample<int>> heartRateStream(String id) => _ctrl<int>(id, 'hr', () => StreamController<VitalSample<int>>.broadcast()).stream;

  @override
  Future<void> startSpO2Notifications(String id) async {
    final c = _ctrl<int>(id, 'spo2', () => StreamController<VitalSample<int>>.broadcast());
    if (!c.hasListener) _tick<int>(id, c, () => 95 + _rnd.nextInt(5));
  }
  @override Future<void> stopSpO2Notifications(String id) async => (_controllers[id]?['spo2'] as StreamController?)?.close();
  @override Stream<VitalSample<int>> spO2Stream(String id) => _ctrl<int>(id, 'spo2', () => StreamController<VitalSample<int>>.broadcast()).stream;

  @override
  Future<void> startTemperatureNotifications(String id) async {
    final c = _ctrl<double>(id, 'temp', () => StreamController<VitalSample<double>>.broadcast());
    if (!c.hasListener) _tick<double>(id, c, () => 36.3 + _rnd.nextDouble() * 0.8);
  }
  @override Future<void> stopTemperatureNotifications(String id) async => (_controllers[id]?['temp'] as StreamController?)?.close();
  @override Stream<VitalSample<double>> temperatureStream(String id) => _ctrl<double>(id, 'temp', () => StreamController<VitalSample<double>>.broadcast()).stream;

  @override
  Future<void> startStepsNotifications(String id) async {
    final c = _ctrl<int>(id, 'steps', () => StreamController<VitalSample<int>>.broadcast());
    if (!c.hasListener) {
      var steps = 1000;
      void loop() {
        if (c.isClosed) return;
  ...
  @override Future<void> stopStepsNotifications(String id) async => (_controllers[id]?['steps'] as StreamController?)?.close();
  @override Stream<VitalSample<int>> stepsStream(String id) => _ctrl<int>(id, 'steps', () => StreamController<VitalSample<int>>.broadcast()).stream;

  @override
  Future<void> startBatteryNotifications(String id) async {
    final c = _ctrl<int>(id, 'batt', () => StreamController<VitalSample<int>>.broadcast());
    if (!c.hasListener) {
      var b = 80;
      void loop() {
        if (c.isClosed) return;
  ...
  @override Future<void> stopBatteryNotifications(String id) async => (_controllers[id]?['batt'] as StreamController?)?.close();
  @override Stream<VitalSample<int>> batteryStream(String id) => _ctrl<int>(id, 'batt', () => StreamController<VitalSample<int>>.broadcast()).stream;

  @override
  Future<void> startRespirationNotifications(String id) async {
    final c = _ctrl<int>(id, 'resp', () => StreamController<VitalSample<int>>.broadcast());
    if (!c.hasListener) _tick<int>(id, c, () => 12 + _rnd.nextInt(8));
  }
  @override Future<void> stopRespirationNotifications(String id) async => (_controllers[id]?['resp'] as StreamController?)?.close();
  @override Stream<VitalSample<int>> respirationStream(String id) => _ctrl<int>(id, 'resp', () => StreamController<VitalSample<int>>.broadcast()).stream;

  @override
  Future<void> startHrvNotifications(String id) async {
    final c = _ctrl<int>(id, 'hrv', () => StreamController<VitalSample<int>>.broadcast());
    if (!c.hasListener) _tick<int>(id, c, () => 20 + _rnd.nextInt(60));
  }
  @override Future<void> stopHrvNotifications(String id) async => (_controllers[id]?['hrv'] as StreamController?)?.close();
  @override Stream<VitalSample<int>> hrvStream(String id) => _ctrl<int>(id, 'hrv', () => StreamController<VitalSample<int>>.broadcast()).stream;

  @override
  Future<void> startBloodPressureNotifications(String id) async {
    final c = _ctrl<BloodPressure>(id, 'bp', () => StreamController<VitalSample<BloodPressure>>.broadcast());
    if (!c.hasListener) _tick<BloodPressure>(id, c, () {
      final s = 110 + _rnd.nextInt(25);
      final d = 70 + _rnd.nextInt(15);
      return BloodPressure(s, d);
    });
  }
  @override Future<void> stopBloodPressureNotifications(String id) async => (_controllers[id]?['bp'] as StreamController?)?.close();
  @override Stream<VitalSample<BloodPressure>> bloodPressureStream(String id) => _ctrl<BloodPressure>(id, 'bp', () => StreamController<VitalSample<BloodPressure>>.broadcast()).stream;
}

final sdkProvider = Provider<SdkService>((ref) => GreenOrangeSdkService());
